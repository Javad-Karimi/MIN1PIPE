function [m, filename, imaxf, imeanf, pixh, pixw, nf, imx1, imn1] = data_cat_modified(path_name, file_base, file_fmt, Fsi, Fsi_new, ratio, ttype)
...
%   Jinghao Lu 01/16/2016
    ...
    if ~contains(file_fmt, 'mat')
        filename = [path_name, file_base, '_reg.mat'];
...
%             pixho = info(1,1).Height;
            ...
            fname_useo = [];
            for ib = 1: nbatch
                reg = zeros(pixh, pixw, idbatch(ib + 1) - idbatch(ib), dtype);
                if contains(file_fmt, 'avi')
                    for i = 1: length(dir_use{ib})
                        ...
                        for ii = rg(rgcount): rg(rgcount + 1) - 1
                            ...
                            reg(:, :, ii - idbatch(ib) + 1) = imresize(frame, [pixh, pixw]);
                            if mod(ii, 100) == 0
                                disp(num2str(ii))
                            end
                        end
...
%                         stt = stt / scl;
                ...
                imeanf = (imeanf * (idbatch(ib) - idbatch(1)) + sum(reg, 3)) / (idbatch(ib + 1) - idbatch(1));
            ...
            imx1 = max(imaxf(:));
            imn1 = min(iminf(:));
            m = normalize_batch(filename, 'reg', imx1, imn1, idbatch);
